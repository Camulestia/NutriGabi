...
vi.mock("@/components/report/report-pdf", () => ({
  ReportPdfDownload: ({ label = "Exportar PDF" }) => <button>{label}</button>
}));
...
describe("ConsultationWizardV3", () => {
  ...
  it("renders step 1 with the consultation identification fields", () => {
    ...
    expect(screen.getByText("A data da consulta será usada para histórico, agenda e lembrete automático de retorno.")).toBeInTheDocument();
  ...
  it("renders step 8 actions and allows saving the professional assessment", async () => {
    ...
    await user.type(screen.getByRole("textbox", { name: "Diagnóstico do profissional" }), "Diagnóstico nutricional inicial");
    await user.type(screen.getByRole("textbox", { name: "Conduta" }), "Plano alimentar e ajuste de rotina");
